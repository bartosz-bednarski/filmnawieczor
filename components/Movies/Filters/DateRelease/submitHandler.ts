...
export const submitHandler:submitHandlerType = ({dateRangeStart,dateRangeEnd,setError,addDateRangeFilter,setDateRangeEnd,setDateRangeStart,onHide}) => {
    if (dateRangeStart === '') {
      setError((state) => ({
        dateRangeStart: true,
    ...
    if (
      ...
      Number(dateRangeStart) >= 0 &&Number(dateRangeStart)>=1900&&Number(dateRangeEnd)>=1900
    ) {
      addDateRangeFilter();
      setDateRangeStart('');
      setDateRangeEnd('');
